...
`desktop` 当前不再把所有源文件都挂在同一条 `part` 链上，而是先把公开、跨层、低 UI 耦合的 core 模块迁成独立 import/export 文件；仍然高度依赖私有 UI helper 的页面与主题文件，暂时保留在 `part` 层。
...
- 当前挂在 `desktop_shell.dart` 下的 UI `part` 文件：
  ...
  - `desktop/lib/src/ui_primitives.dart`
  - `desktop/lib/src/app_models.dart`（当前仅保留 UI 私有结构）
...
- `app_copy.dart` 依赖 `_SettingsCategory`
- `desktop_app.dart` 同时协调 settings / workspace 两个 UI 分支
- `app_theme.dart`、`ui_primitives.dart`、settings/workspace 组件文件之间共享大量 `_AppTypography`、palette、density helper
