...
- 任务：对当前 Pi App 已声明交付的能力做分层验收，建立可复现缺陷清单与修复优先级；ACC-0 已完成，后续 ACC-A 至 ACC-E 是持续执行的基线验证活动，并可在确认 S0/S1 时重新阻断后续 I2 / C1 实现。
...
| S1 | 核心已交付路径无法完成、错误状态不可恢复、runtime 诊断误导用户、项目/偏好持久化损坏、abort 后无法继续工作。 | 在继续 I2 / C1 前修复。 |
...
- 验收重点：确认 S1 时停止 ACC-B、ACC-C、ACC-D 和后续 I2 / C1 新实现，先完成修复与聚焦回归；确认非缺陷时记录交互契约和证据后继续。
...
- 验收重点：每个失败项有 owner、修复优先级、回归用例和是否阻断 I2 / 发布的结论。
...
- 完成标准：通过则记录证据并进入 ACC-B；失败则登记 S1、暂停后续验收和 I2 新实现，另建最小修复单元。
