...
- 状态：已拆分（已完成基础设施收口；剩余内部边界复核与兼容层清理待总看板重新排期）
- 负责人：Pi
- 日期：2026-07-26
- 承接计划：`docs/plans/2026-07-26-desktop-persistence-and-modularization.md`
- 当前计划入口：`docs/plans/README.md`

> **文档状态，2026-07-28：** 阶段 1-3 已作为当前 desktop 基线交付。阶段 4-5 不是独立执行队列；只有在主路线图评估其对 I1、P1、I2、C1 或后续工作有明确价值时，才以新的、范围更小的执行单元重新排期。
...
- 已完成：阶段 3 / 单元 9-12 第三批
  - 左侧 `Projects` 区从展开块收成更接近真实桌面客户端的扁平项目列表
  ...
  - 每个注册项目会在 `~/.pi-app/projects/<project-id>/project.json` 保存显示别名；`…` 菜单可重命名，且不会改动源目录名
  - 应用默认启动不再把自身运行目录或容器 `Data` 目录误识别成项目
- 已拆分：阶段 4-5 的 feature 内部边界复核、兼容层与过渡文件清理需要在主路线图中以独立、范围更小的单元重新排期；当前不得从本文件直接创建执行任务。
